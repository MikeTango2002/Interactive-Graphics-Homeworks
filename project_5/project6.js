...
vec3 Shade( Material mtl, vec3 position, vec3 normal, vec3 view )
{
	...
	for ( int i=0; i<NUM_LIGHTS; ++i ) {
		...
		vec3 omega = normalize(lights[i].position);
		vec3 n = normalize(normal);
		vec3 h = normalize(omega + v);
		//
		float cos_theta = max(dot(n, l), 0.0);
		float cos_fi = max(dot(n, h), 0.0);

		color += lights[i].intensity * (cos_theta * mtl.k_d + mtl.k_s * pow(cos_fi, mtl.n));	
	}
	return color;
}
...
bool IntersectRay( inout HitInfo hit, Ray ray )
{
	...
	for ( int i=0; i<NUM_SPHERES; ++i ) {
		...
		float two_times_a = 2 * a;
		float t1 = (-b + sqrt_delta) / two_times_a;
		float t2 = (-b - sqrt_delta) / two_times_a;

		float t = t1<t2 ? t1:t2;

		if (t < hit.t){
			hit.t = t;
			hit.position = pos + dir*t;
